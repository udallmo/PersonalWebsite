...
module.exports = {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Content._attributes)
      .filter(attribute => attribute !== Content.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Content._attributes[attribute].type));
