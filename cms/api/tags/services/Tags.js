...
module.exports = {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Tags._attributes)
      .filter(attribute => attribute !== Tags.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Tags._attributes[attribute].type));
